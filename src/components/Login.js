      ...
      if (res.data.status === "fail") {
        ...
        if (res.data.status === "success") {
          localStorage.setItem(
            "eatable",
            JSON.stringify({ accessJWT: res.data.accessJWT })
          );
        }
        ...
        if (res.data.type === "1") {
          ...
          const url =
            process.env.SELLER_ROUTE || "http://localhost:8080/seller";
          // const url = '/seller'
          const seller = await axios.get(url + `/${id}`, {
            headers: JSON.parse(localStorage.getItem("eatable")),
          });
          if (
            seller.data[0].shop_name === null ||
            seller.data[0].opening_time === null ||
            seller.data[0].shop_location === null
          ) {
        ...
        <Row>
          <Col>
            <p className="global-stats">
              ...
              <span className="global-stats-servings">
                {soldItems.length} Servings
              </span>
            </p>
          </Col>
          <Col>
            ...
            <div className="w-100 text-center mt-2 lp-guide-text">
              Need an account?{" "}
              <Link to="/" className="lp-guide-link">
                Sign Up
              </Link>
            </div>
          </Col>
