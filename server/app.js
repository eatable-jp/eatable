...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).send("Unable to register user").end();
  }
});
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (data.length === 0) {
      ...
      bcrypt.compareSync(newUser.password, data[0].password) === false
    ) {
    ...
    console.error(err);
    res.status(500).send("Entry Not Found").end();
  }
});
...
const getSoldItems = async (pool) => {
  return await pool.select("id", "conformation").from("items");
};
...
const getItems = async (pool) => {
  return await pool.select("*").from("items");
};
  ...
  try {
    return await pool("items")
      ...
      .update({
        ...
        note: newItem.note,
        buyer_id: newItem.buyer_id,
        conformation: newItem.conformation,
      });
  } catch (err) {
    throw Error(err);
  }
...
const getSellers = async (pool) => {
  return await pool.select("*").from("sellers");
};
...
app.get("/seller/:id", async (req, res) => {
  ...
  try {
    ...
    const retSeller = sellers.filter(
      (seller) => seller.id === parseInt(paramsId)
    );
...
app.post("/seller", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: "success",
      message: "seller added Successful",
      id: data[0],
    });
  } catch (err) {
    console.error(err);
...
const updateSeller = async (pool, newSeller, sellerId) => {
  try {
    return await pool("sellers")
      .where({ id: sellerId })
      .update({
        seller_name: newSeller.seller_name,
        shop_name: newSeller.shop_name,
        shop_location: newSeller.shop_location,
        shop_long: newSeller.shop_long,
        shop_lat: newSeller.shop_lat,
        opening_time: newSeller.opening_time,
        closing_time: newSeller.closing_time,
        phone_number: newSeller.phone_number,
        email_address: newSeller.email_address,
      });
  } catch (err) {
    throw Error(err);
  }
...
const getBuyers = async (pool) => {
  return await pool.select("*").from("buyers");
};
...
app.post("/buyer", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: "success",
      message: "Buyer added Successful",
      id: data[0],
    });
  } catch (err) {
    console.error(err);
...
const updateBuyer = async (pool, newBuyer, buyerId) => {
  try {
    return await pool("buyers")
      .where({ id: buyerId })
      .update({
        buyer_name: newBuyer.buyer_name,
        buyer_address: newBuyer.buyer_address,
        phone_number: newBuyer.phone_number,
